...
const FullViewModal = ({
  ...
  const [updateExampleIndex, { isLoading: isExampleLoading }] =
    useUpdateExampleIndexMutation();

  useEffect(() => {
    setUserQuery(currentExample.user_query);
    setInsightsSteps(currentExample.steps_to_follow);
    setInsightsSql(currentExample.sql_cmd);
    setNewSqlQuery(currentExample.sql_cmd);
  }, [currentExample, show]);
  ...
  const tableHeaders = Object.keys(showInsightTable).map((key, index) => (
    <th
      key={index}
      className="px-4 py-2 text-xs font-normal tracking-wide border-t border-l border-r text-secondary-text border-border-color whitespace-nowrap"
    >
      {formatKey(key)}
    </th>
  ));
      ...
      <tr key={rowIndex}>
        {Object.keys(showInsightTable).map((key, colIndex) => (
          <td
            ...
            className="px-4 py-2 text-xs tracking-wide border-r text-primary-text bg-page border-border-color whitespace-nowrap"
          >
  ...
  const handleSaveChanges = () => {
    ...
    updateExampleIndex({
      datasource_id: slug,
      source_id: currentExample.source_id,
      payload: payload,
    }).then((response) => {
            ...
            <div className="flex flex-col space-y-3">
                ...
                <div className="flex items-center space-x-4">
                  {currentTab === "response" || (
                    <button
                      ...
                      disabled={activeRun || executeSqlLoading}
                    >
                      <span className="flex items-center justify-center">
                        {executeSqlLoading || (
                          <span className="flex items-center justify-center">
                      ...
                      <span className="font-medium whitespace-nowrap">Run</span>
                    </button>
                  )}
                </div>
              </div>
              ...
              {currentTab === "response" &&
                Object.keys(showInsightTable).length > 0 && (
                  <div className="flex flex-col w-full p-2 space-y-4 rounded-md bg-page">
                    <div className="flex flex-col w-full overflow-x-auto recent__bar">
                      <table className="w-full min-w-full text-sm border-b divide-y-2 rounded-md border-x divide-border-color bg-page-hover border-border-color">
                        <thead className="text-left ltr:text-left rtl:text-right bg-page-hover">
                          <tr>{tableHeaders}</tr>
                        </thead>
                      ...
                      {totalItems > 0 || (
                        <div className="flex items-center justify-center w-full h-48 rounded-br-md rounded-bl-md bg-[#2A2D34]">
              ...
              {currentTab === "error" && (
                <div className="flex flex-col p-4 overflow-hidden overflow-y-auto text-sm leading-6 border rounded-md max-h-72 min-h-32 recent__bar bg-page border-border-color font-roboto">
                  <ul className="flex flex-col space-y-4">
                    <li className="flex space-x-4 text-sm tracking-wide text-white">
                      ...
                      <span className="w-full font-medium leading-5 text-white">
                        {sqlError}
                      </span>
                    </li>
                  </ul>
