...
import { useAddExampleToIndexMutation } from "@/store/datasource";
...
const DatasourceSave = ({
  ...
  const [
    addExampleToIndex,
    { error: addExampleError, isLoading: addExampleLoading },
  ...
  useEffect(() => {
    const initialQuery =
      eventStats?.generation?.user_query ||
      eventStats?.regenerate?.user_query ||
      "";
    const initialSteps =
    ...
    setStepsToFollow(initialSteps);
    setInitialData({
      userQuery: initialQuery,
      stepsToFollow: initialSteps,
      currentSql: currentSql,
    });
  }, [eventStats, currentSql]);

  // const isSaveDisabled =
  //   userQuery === initialData.userQuery &&
  //   stepsToFollow === initialData.stepsToFollow &&
  //   currentSql === initialData.currentSql;

  const isSaveDisabled =
    !userQuery.trim() || !stepsToFollow.trim() || addExampleLoading;
        ...
        <div className="relative flex flex-col">
          <div className="sticky top-0 left-0 flex items-center justify-between w-full px-4 py-3 border-b bg-page border-border-color">
            <div className="flex items-center space-x-4 text-xs font-semibold tracking-wide text-primary-text">
              ...
              <span className="text-primary-text">Response Summary</span>
            </div>
          ...
          <div className="flex flex-col px-4 py-4 space-y-3">
            ...
            <input
              type="text"
              className="w-full h-12 px-4 py-3 text-xs font-medium border rounded-md outline-none bg-page text-input-text 2xl:text-sm border-input-border focus:border-input-border-focus"
              value={userQuery}
            ...
            <textarea
              className="w-full px-4 py-3 text-xs font-medium border rounded-md outline-none h-28 bg-page border-border-color min-h-28 text-secondary-text focus:border-input-border-focus"
              value={stepsToFollow}
              onChange={(e) => setStepsToFollow(e.target.value)}
            ></textarea>
