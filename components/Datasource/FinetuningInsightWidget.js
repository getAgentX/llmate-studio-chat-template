...
const FinetuningInsightWidget = ({
  ...
  const [addWidgetFromSql, {}] = useAddWidgetFromSqlMutation();
  ...
  return (
    <div className={`relative flex justify-end ${className}`}>
      <div className="relative">
        ...
        {toggleDashboard && (
          <div
            className="absolute top-8 right-6 h-64 w-[300px] rounded-md bg-dropdown-bg border border-border-color z-50 shadow-sm"
            ref={dashboardRef}
          >
                ...
                <div className="flex-1 overflow-y-auto recent__bar">
                  <ul className="flex flex-col h-full space-y-1 text-xs font-normal text-muted">
                    {fetchedDashboards?.map((item, index) => {
                      return (
                        <li
                          className={`flex items-center justify-between py-2 px-2 hover:bg-btn-primary cursor-pointer texy-primary-text hover:text-white ${
                            currentDashboardId === item.id
        ...
        {toggleSection && (
          <div
            className="absolute top-8 right-[328px] h-64 w-[300px] rounded-md bg-dropdown-bg border border-border-color z-50 shadow-sm shadow_right"
            ref={sectionRef}
          >
            <div className="flex flex-col h-full space-y-2">
              <div className="flex flex-col flex-1 space-y-2 overflow-y-auto recent__bar">
                  ...
                  {fetchedSections?.length > 0 && (
                    <ul className="flex flex-col h-full space-y-1 text-xs font-normal text-muted">
                      {fetchedSections?.map((item, index) => {
                        return (
                          <li
                            className="flex items-center justify-between px-2 py-1 bg-transparent cursor-pointer h-9 group hover:bg-btn-primary texy-primary-text hover:text-white"
                            key={index}
...
export default FinetuningInsightWidget;
