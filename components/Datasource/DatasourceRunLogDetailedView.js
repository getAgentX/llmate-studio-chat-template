...
import { useVerifySQLDatasourceRunLogMutation } from "@/store/sql_datasource";
...
const DatasourceRunLogDetailedView = ({
  show,
  setShow,
  logId,
  datasourceId,
  currentRunLog = {},
  setShowLogsSuccessModal,
}) => {
  ...
  useEffect(() => {
    if (currentRunLog.verification_status === "VERIFIED") {
      setUserQuery(
        currentRunLog.verified_data?.user_query || currentRunLog.user_query
      );
      setInsightsSteps(
        currentRunLog.verified_data?.final_steps_to_follow ||
          currentRunLog.final_steps_to_follow
      );
      setInsightsSql(
        currentRunLog.verified_data?.final_sql || currentRunLog.final_sql
      );
      setNewSql(
        currentRunLog.verified_data?.final_sql || currentRunLog.final_sql
      );
      ...
      setUserQuery(currentRunLog.user_query);
      setInsightsSteps(currentRunLog.final_steps_to_follow);
      setInsightsSql(currentRunLog.final_sql);
      setNewSql(currentRunLog.final_sql);

      if (currentRunLog.status === "SUCCESS") {
        setButtonDisabled(false);
      } else {
  ...
  const [
    verifySQLDatasourceRunLog,
    { isLoading: verifyLoading, error: verifyError },
  ] = useVerifySQLDatasourceRunLogMutation();

  const handleVerifyAndSaveChanges = () => {
    verifySQLDatasourceRunLog({
      datasource_run_log_id: logId,
      status: "VERIFIED",
      verified_data: {
        user_query: userQuery,
        final_steps_to_follow: insightsSteps,
        final_sql: insightsSql,
      },
    }).then((response) => {
      if (response) {
        setShowLogsSuccessModal(true);
        resetSidebar();
      }
    });
  };
  ...
  return (
    ...
    >
      <div
        className="fixed top-0 bottom-0 right-0 w-full !z-[10000] max-w-2xl overflow-x-hidden overflow-y-auto border-l border-border-color bg-page recent__bar"
        ref={modalRef}
      >
        <div className="relative flex flex-col">
          <div className="sticky top-0 left-0 flex items-center justify-between w-full h-12 px-4 border-b bg-page border-border-color">
            ...
            <div className="flex items-center space-x-3">
              <div className="flex items-center px-2 py-1 space-x-2 text-xs font-medium rounded-md text-primary-text">
                <span className="text-xs font-normal text-secondary-text">
                  Run Status :
                </span>

                <div>
                  {currentRunLog.status === "SUCCESS" && (
                    <span className="text-xs font-medium text-[#1BC655]">
                      Success
                    </span>
                  )}

                  {currentRunLog.status === "FAILED" && (
                    <span className="text-xs font-medium text-[#C61B1B]">
                      Failed
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center space-x-2 text-xs font-medium text-primary-text">
                <span className="text-xs font-normal text-secondary-text">
                  Verification Status :
                </span>

                {currentRunLog.verification_status === "VERIFIED" ? (
                  <span className="text-[#1BC655] text-xs font-medium">
                    Verified
                  </span>
                ) : (
                  <span className="text-[#B15F15] text-xs font-medium">
                    Pending
                  </span>
                )}
              </div>
              ...
              <button
                ...
                disabled={buttonDisabled || verifyLoading}
                onClick={handleVerifyAndSaveChanges}
                data-tooltip-id="tooltip"
                data-tooltip-content={
                  buttonDisabled ? "Run SQL succesfully to verify" : null
                }
          ...
          <div className="flex flex-col p-4 space-y-3">
            ...
            <div className="flex flex-col space-y-3">
                ...
                <div className="flex items-center w-full space-x-2">
                  ...
                  {sqlError && (
                    <button
                      className={`text-xs h-8 border-b-2 font-medium flex justify-center items-center space-x-2 tracking-wider capitalize transition-colors duration-300 ${
                        currentTab === "error"
                          ? "text-primary-text border-secondary"
                ...
                <div className="flex items-center space-x-4">
                  {currentTab === "response" || (
                    <button
                      ...
                      disabled={executeSqlLoading || disableRun}
                    >
                      {executeSqlLoading || (
                        <span className="flex items-center justify-center">
                          <svg
              ...
              {currentTab === "sql" && (
                <div className="overflow-hidden border rounded-md border-border-color">
                  <Editor
                    key={customTheme}
                    height="200px"
              ...
              {currentTab === "error" && (
                <div className="flex flex-col px-4 py-4 overflow-hidden overflow-y-auto text-xs leading-6 border rounded-md border-border-color max-h-72 min-h-32 recent__bar bg-page font-roboto">
...
export default DatasourceRunLogDetailedView;
