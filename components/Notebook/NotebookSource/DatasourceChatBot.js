...
import DatasourceResizableTable from "@/components/Datasource/DatasourceResizableTable";
import DatasourcePivotGraph from "@/components/Datasource/DatasourcePivotGraph";
...
const Options = {
  // copyWithSyntaxHighlighting: false,  // Ensures no styling is copied
...
const DatasourceChatBot = ({
  ...
  const [getDatasource, { data: getSqlRes }] = useGetDatasourceMutation();
  const [stopDatasourceRun, { isLoading: stopDatasourceLoading }] =
    useStopDatasourceRunMutation();

  useEffect(() => {
    if (getSqlRes) {
      setCurrentDatasource(getSqlRes);
    }
  }, [getSqlRes]);
  ...
  const [getDatasourceRun, {}] = useGetDatasourceRunMutation();
  ...
  useEffect(() => {
    if (id && eventStats.run_id) {
      getDatasourceRun({
        datasource_id: id,
        run_id: eventStats.run_id,
      }).then((response) => {
        if (response.data) {
          const eventsObject = response.data.events;
          const eventsArray = Object.values(eventsObject);

          setQuery(response.data.run_input.user_query);

          setEventStats((prev) => ({
            ...prev,
            run_id: response.data._id,
          }));

          if (response.data.events && eventsArray.length > 0) {
    ...
    const datasourceInitialData = normalizedInitialData?.filter((item) => {
      if (source_block) {
    ...
    if (
      datasourceInitialData.length > 0 &&
      datasourceInitialData[0].props?.data?.datasource_run_id &&
      datasourceInitialData[0].props?.data?.event_id
    ) {
  ...
  useEffect(() => {
    if (id) {
      handleResetEvents();
      setSlug(id);
      getDatasource({ datasource_id: id });
    }
  }, [id]);
  ...
  if (
    ...
    getSqlRes?.ds_config?.last_transformation_synced_at === null
  ) {
    return (
      <div className="w-full h-full">
        <div className="relative flex flex-col flex-1 h-full">
          <div className="sticky top-0 left-0 z-50 flex items-center w-full h-12 py-3.5 px-4 space-x-2 border-b bg-page border-border-color">
            <span className="flex items-center justify-center">
              {customTheme === "dark" ? (
                <svg
                  viewBox="0 0 20 20"
                  xmlns="http://www.w3.org/2000/svg"
                  className="w-5 h-5 stroke-icon-selected-color"
                >
                  <path
                    d="M18.3308 4.58301C18.3308 4.58301 18.3307 15.0256 18.3307 15.833C18.3307 17.2137 14.5998 18.333 9.9974 18.333C5.39502 18.333 1.66408 17.2137 1.66408 15.833C1.66408 15.059 1.66406 4.58301 1.66406 4.58301"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M18.3307 12.083C18.3307 13.4637 14.5998 14.583 9.9974 14.583C5.39502 14.583 1.66406 13.4637 1.66406 12.083"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M18.3307 8.33301C18.3307 9.71372 14.5998 10.833 9.9974 10.833C5.39502 10.833 1.66406 9.71372 1.66406 8.33301"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M9.9974 6.66699C14.5998 6.66699 18.3307 5.5477 18.3307 4.16699C18.3307 2.78628 14.5998 1.66699 9.9974 1.66699C5.39502 1.66699 1.66406 2.78628 1.66406 4.16699C1.66406 5.5477 5.39502 6.66699 9.9974 6.66699Z"
                    className="fill-icon-selected-color"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                </svg>
                ...
                <svg
                  ...
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                  className="w-4 h-4 stroke-icon-selected-color"
                >
                  <path
                    d="M18.3308 4.58301C18.3308 4.58301 18.3307 15.0256 18.3307 15.833C18.3307 17.2137 14.5998 18.333 9.9974 18.333C5.39502 18.333 1.66408 17.2137 1.66408 15.833C1.66408 15.059 1.66406 4.58301 1.66406 4.58301"
                    stroke="#010616"
            ...
            <span className="text-sm font-medium tracking-wider capitalize text-primary-text">
              {getSqlRes?.name || "Datasource Name"}
            </span>
          </div>
          ...
          <div className="flex items-center justify-center w-full h-full max-h-full text-white">
            <div className="flex flex-col items-center w-full max-w-full">
              ...
              <p className="w-full max-w-xl mx-auto text-sm font-normal text-center text-secondary-text">
                Please wait while your data is syncing. You will be notified
                once your datasource is available
              </p>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="w-full h-full">
      <div className="relative flex flex-col flex-1 h-full">
        {showThoughts || (
          <div className="sticky top-0 left-0 z-50 flex items-center justify-between w-full min-h-12 max-h-12 h-full py-3.5 px-4 space-x-2 border-b bg-page border-border-color">
            {showDropdown ? (
              <div
        ...
        {showThoughts && (
          <div className="sticky top-0 left-0 flex items-center justify-between w-full h-12 px-4 border-b min-h-12 bg-page border-border-color">
          ...
          {showThoughts || (
            ...
            >
              <div className="z-10 flex flex-col w-full h-full max-h-full">
                ...
                ) : queryLoading === false && showResult === false ? (
                  <div className="flex flex-col flex-1 w-full h-full py-2 overflow-hidden">
                    <div className="flex flex-col items-center justify-center w-full h-full space-y-2 text-primary-text">
                      ...
                      <p className="text-xs font-semibold 2xl:text-sm text-secondary-text">
                        Submit your query above to generate the SQL here
                      </p>
                    </div>
                  </div>
                ) : queryLoading ? (
                ...
                {showResult || (
                  <div className="flex flex-col flex-1 w-full h-full py-2 overflow-hidden">
                    <div className="flex flex-col items-center justify-center w-full h-full space-y-2 text-primary-text">
                      ...
                      <p className="text-xs font-semibold 2xl:text-sm text-secondary-text">
                        Start typing your query in the input box above to
                        generate and view the data instantly.
                      </p>
                    </div>
                  </div>
                )}
...
export default DatasourceChatBot;
