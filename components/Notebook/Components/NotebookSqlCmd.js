...
import { useUpdateBlocksMutation } from "@/store/notebook";
...
const NotebookSqlCmd = ({
  ...
  function mapEditorBlockToBackendBlock(block) {
    const backendBlock = {
      block_id: block.id,
      block_type:
      ...
      children: block.children
        ? block.children.map(mapEditorBlockToBackendBlock)
        : [],
    };

    if (
      block.type === "sql_datasource_run" ||
      block.type === "chatbot_sql_datasource_run"
    ) {
      // Keep copies of block_id and block_type
      const { block_id, block_type } = backendBlock;

      // Merge the fields from `block.props.data` into `backendBlock`
      // which overwrites fields on `backendBlock` if they collide
      Object.assign(backendBlock, block.props.data);

      // Restore the block_id and block_type
      backendBlock.block_id = block_id;
      backendBlock.block_type = block_type;

      // Optionally remove props, content, and children from the final object
      backendBlock.props = undefined;
      backendBlock.content = undefined;
      backendBlock.children = undefined;
    }

    return backendBlock;
  }

  const blocksStructure = blocksDocuments?.map(mapEditorBlockToBackendBlock);

  const router = useRouter();

  const { sqlCmd, setSqlCmd, blockId, datasoureId } =
  ...
  const [updateBlocks, { isLoading: isExampleLoading }] =
  ...
  const handleSaveChanges = () => {
    ...
    if (blocksStructure?.length > 0) {
      const updatedBlocks = blocksStructure.map((block) => {
