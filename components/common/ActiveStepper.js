...
const ActiveStepper = ({ currentStepIndex, totalSteps, goTo }) => {
              ...
              <div className="relative flex flex-col items-center">
                <div
                  className={`w-7 h-7 flex items-center justify-center rounded-full p-1.5 ${
                    isActive || isCompleted ? "bg-secondary" : "bg-[#26282D]"
                  } cursor-pointer`}
                  onClick={() => goTo(index)}
                >
                  {isCompleted ? (
                ...
                <div
                  className={`absolute w-32 mt-4 text-xs font-medium text-center top-full tracking-wide cursor-pointer ${
                    isActive
