...
const OrgInfoSidebar = ({
  ...
  const {
    data: allOrgList,
    error: allOrgListError,
    isLoading: allOrgListLoading,
    isFetching: allOrgListFetch,
  } = useGetAllOrgLevelAccountsQuery({});

  const transformAllUserListToArray = (allOrgList) => {
    return Object.keys(allOrgList).map((connectorType) => {
      const accountsObj = allOrgList[connectorType] || {};
  ...
  useEffect(() => {
    if (allOrgList) {
      const arrayBasedData = transformAllUserListToArray(allOrgList);
...
const Database = ({ data, isActive, onToggle, router }) => {
  const handlePageChange = () => {
    router.push(`/org-info/${data.connector_type}`);
  };

  function removeUnderscoresAndDashes(str) {
    return str.replace(/[_-]/g, " ");
  }

  const cleanedString = removeUnderscoresAndDashes(data.connector_type);
      ...
      {isActive && (
        <ul className="flex flex-col w-full">
          {data.accounts.map((account) => (
            <Link
              key={account.id}
              href={`/org-info/${data.connector_type}/s/${account.id}`}
