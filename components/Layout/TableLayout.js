...
const TableLayout = ({ children, activeTab, extraChild = null }) => {
  ...
  useEffect(() => {
    if (router?.query?.slug) {
      ...
      setTableName(router?.query.table);
    }
  }, [router?.query?.slug]);
          ...
          <div className="sticky top-0 left-0 z-50 flex flex-col w-full bg-page">
            <div className="flex items-center justify-between h-12 px-4 border-b border-border-color">
              <div className="flex items-center space-x-2">
                ...
                <div className="flex items-center space-x-2">
                  <p className="text-base font-normal capitalize text-primary-text">
                    {router?.query?.table || ""}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <Link
                  href={`/datasource/details/${slug}/query/${router.query.table}`}
                ...
                {tabsList?.map((item, index) => {
                  return (
                    <Link
                      ...
                      href={`/datasource/details/${slug}/table/${tableName}${item.slug}`}
                    >
                      {item.name}
                    </Link>
                  );
