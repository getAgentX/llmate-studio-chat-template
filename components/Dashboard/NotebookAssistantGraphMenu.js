...
import { useUpdateDataVisualizationConfigMutation } from "@/store/message";
...
const NotebookAssistantGraphMenu = ({
  show,
  setShow,
  dataField,
  event = {},
  ...
  const [visualizationConfig, setVisualizationConfig] = useState(config || {});
  ...
  const [updateDataVisualizationConfig, {}] =
    useUpdateDataVisualizationConfigMutation();

  const { data, error, isFetching } = useGetAssistantGraphQuery({
      ...
      if (messageId && event.id) {
        updateDataVisualizationConfig({
          ...
          if (response.data === null) {
            setFields([]);
            setChartData({});
            setvisualizationConfig(visualizationConfig);
            setVisualizationConfig([]);
                ...
                <div className="col-span-5 overflow-hidden">
                  <div className="flex flex-col h-full max-h-full space-y-2">
                    <div className="flex items-center sticky top-0 left-0 z-[100] bg-primary justify-between px-4 pt-3 pb-2.5 border-b border-border">
                      <div className="flex items-center space-x-3 text-xs font-medium text-primary-text capitalize">
...
export default NotebookAssistantGraphMenu;
