...
import { ChatEvents } from "@/components";
import useAutoResizeTextArea from "@/hooks/useAutoResizeTextArea";
import { useTheme } from "@/hooks/useTheme";
import { useRouter } from "next/router";
import { useAddWidgetFromMessageMutation } from "@/store/dashboard";

const AssistantAnalyzer = ({
  currentAssistant: assistantData,
  setShow = () => {},
  currentSection = {},
  refetchDashboard = () => {},
}) => {
  ...
  const [
    getAssistant,
    { isLoading: assistantLoading, error: getAssistantError },
  ] = useGetAssistantMutation();
  const [stopMessageResponse, { error: stopMessageResponseError }] =
    useStopMessageResponseMutation();
  const [getChats, { isLoading: chatsLoading, error: chatsError }] =
    useGetChatsMutation();

  const refetchAssistant = (assistant_id) => {
    getAssistant({ assistant_id }).then((response) => {
      if (response.data) {
        const responseData = response.data;
        setCurrentAssistant(responseData);
        setAssistantName(responseData.name);
      }
    });
  };
  const fetchChatHistoryList = async (assistantId) => {
    try {
  ...
  useEffect(() => {
    ...
    if (assistant_id) {
      refetchAssistant(assistant_id);
      fetchChatHistoryList(assistant_id);
    }
  }, [assistantData]);
  ...
  const handleSendMessage = (message) => {
    ...
    if (createdChatId) {
      ...
      createChat({ assistant_id: assistantData.id }).then((response) => {
        if (response.data) {
          if (response.data.id) {
        ...
        getMessageById({
          ...
          if (response.data) {
            ...
            if (responseInfo.error_message) {
              ...
              //   if (createdChatId) {
              //     router.push(
              //       `/assistant/chat/${assistantData.id}/history/${createdChatId}`
              //     );
              //   }
            } else {
  ...
  const handleAddWidgetFromEvent = (event, messageId) => {
    const payload = {
      message_id: messageId,
      event_id: event.id,
      label: event.user_query,
    };
    ...
    <>
      {history?.length > 0 && (
        <div className="relative flex flex-col items-center w-full h-full">
          <div
            ref={scrollRef}
            className="w-full h-full px-6 pb-16 my-2 space-y-3 overflow-y-auto recent__bar scroll-smooth"
          >
                  ...
                  <div className="bg-page w-[97%]" key={index}>
                    <div className="flex flex-col w-full mx-auto space-y-2">
                      ...
                      <div className="flex ml-10">
                        <UserFeedback messageId={messageId} />
                      </div>
                    </div>
                  </div>
                );
          ...
          <div className="sticky left-0 right-0 flex items-center justify-center w-full px-6 pb-2 bottom-2">
...
export default AssistantAnalyzer;
