...
import { Db_type, getDbTypeImage } from "./AssistantsTable";
...
const DashboardTable = ({
  ...
  const columns = ["Name", "Description", "Datasources used", "Action"];
  ...
  const tableRows = data?.map((item, index) => {
    ...
    return (
      ...
      >
        ...
        <td className="px-3 text-[13px] font-normal text-left border-t cursor-pointer h-12 border-border-color text-primary-text max-w-[270px] overflow-hidden text-ellipsis whitespace-nowrap">
          ...
          <div className="flex items-center space-x-1">
            <div className="flex items-center space-x-0.5">
              ...
              {allDataSources.length > 3 && (
                <span className="flex items-center justify-center w-8 h-8 rounded-full border border-border-color bg-page text-[13px] font-medium text-primary-text">
                  +{allDataSources.length - 3}
                </span>
              )}
            </div>
    ...
    return (
      <div className="flex flex-col items-center w-full max-w-full mx-auto rounded-md py-10">
        ...
        <p className="mt-2 text-lg font-medium text-primary-text">
          You haven’t accessed any dashboards yet. Get started by:
        </p>
        <p className="w-full max-w-2xl mx-auto text-base font-normal text-center text-secondary-text">
          Creating a New Dashboard to track your key metrics. Exploring Existing
          Dashboards to gain insights instantly. Take control of your data
          today!
        </p>
        <button
          type="button"
        ...
        >
          ...
          <span className="hidden xsm:block">Create Dashboard</span>
    ...
    return (
      <div className="flex flex-col items-center w-full max-w-full mx-auto rounded-md py-10">
        ...
        <p className="w-4/5 mt-1 max-w-2xl mx-auto text-[13px] font-normal text-center text-secondary-text">
          Dashboards provide a visual way to monitor your data and insights.
          Start by creating a dashboard that suits your needs or explore
          existing templates to customize one.
        </p>
  ...
  if (searchQuery && data.length === 0) {
    // Dashboards exist, but search yields no results
    return (
