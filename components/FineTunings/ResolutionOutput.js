...
const ResolutionOutput = ({ eventStats }) => {
  ...
  return (
    <>
      <div className="flex flex-col space-y-4 sm:space-y-6">
        {eventStats.regenerate?.regenerated_steps_to_follow && (
          <div className="flex flex-col space-y-2">
            ...
            <textarea
              ...
              value={eventStats.regenerate.regenerated_steps_to_follow}
            />
          </div>
        )}
          ...
          <div className="flex flex-col space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center w-full space-x-2">
                {eventStats.regenerate_execution?.sql && (
                  <a
                    className={`px-4 py-2 text-xs font-medium flex items-center space-x-2 justify-center tracking-wide rounded-md cursor-pointer ${
                      editSqlTab === "sql"
                        ? "bg-[#26282D] text-white"
                        : "text-white/40 bg-[#1C1E21]"
                    }`}
                  ...
                  >
                    ...
                    <span className="tracking-wider">Re-Generated SQL</span>
                  </a>
                )}

                <a
                  className={`px-4 py-2 text-xs font-medium tracking-wide flex items-center space-x-2 justify-center rounded-md cursor-pointer ${
              ...
              {eventStats.regenerate_execution?.error_message && (
                <div
            ...
            {eventStats.regenerate?.regenerated_sql && editSqlTab === "sql" && (
              <div className="overflow-hidden border rounded-md border-border">
                <SyntaxHighlighter
                  ...
                  wrapLongLines={true}
                >
                  {eventStats.regenerate.regenerated_sql}
                </SyntaxHighlighter>
              </div>
            )}
            ...
            {editSqlTab === "data-response" && (
              <TableInformation
                data={eventStats.regenerate_execution?.dataframe || {}}
              />
            )}
          </div>
        )}
      ...
      {eventStats.regenerate_execution && (
        <SqlErrorModal
          ...
          data={[eventStats.regenerate_execution?.error_message]}
        />
      )}
    </>
  );
...
export default ResolutionOutput;
