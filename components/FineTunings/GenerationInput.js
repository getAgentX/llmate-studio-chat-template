...
const GenerationInput = ({ eventStats }) => {
  ...
  const handleGenerationSelect = (value) => {
    const selectedData = eventStats.generation?.examples_used.find(
  ...
  useEffect(() => {
    const options = eventStats.generation?.examples_used?.map((example) => {
      return {
    ...
    const currIndex = options?.findIndex((item) => {
      return item.value === eventStats.generation?.examples_used[0].user_query;
    });
  ...
  if (eventStats.generation?.examples_used?.length === 0) {
    return (
      ...
      <div className="flex flex-col w-full p-2 space-y-4 border rounded-md sm:p-4 border-[#212227] bg-page">
        <div className="flex flex-col space-y-4">
          ...
          <SelectOption
            ...
            defaultValue={{
              value: eventStats.generation?.examples_used[0].user_query,
              label: eventStats.generation?.examples_used[0].user_query,
            }}
        ...
        <div className="flex flex-col space-y-4">
          ...
          <textarea
            readOnly
            type="text"
            className="w-full px-4 py-3 overflow-y-auto text-sm text-white border rounded-md bg-page outline-none resize-none h-36 border-[#212227] placeholder:text-white/40 recent__bar"
